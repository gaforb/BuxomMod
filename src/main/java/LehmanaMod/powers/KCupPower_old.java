...
import LehmanaMod.cards.BrokenBraK;
import LehmanaMod.util.TextureLoader;
import com.megacrit.cardcrawl.actions.common.ReducePowerAction;
import com.megacrit.cardcrawl.actions.common.MakeTempCardInDrawPileAction;

public class KCupPower_old extends AbstractPower implements CloneablePowerInterface {
    public AbstractCreature source;

    public static final String POWER_ID = DefaultMod.makeID("KCupPower_old");
    ...
    public void onApplyPower(AbstractPower power, AbstractCreature target, AbstractCreature source) { // At the end of your turn
        if (power instanceof CommonPower) {
            if ((target.getPower("LehmanaMod:CommonPower") != null) && (target.getPower("LehmanaMod:CommonPower").amount >= 5)) {
                flash();
    ...
    public AbstractPower makeCopy() {
        return new KCupPower_old(owner, source, amount);
    }
}
